%% QT_JBP.m: Jacobi Belief propagation algorithm for Quantum 
...
function runhist = QT_JBP(G,C,c,par) 
tstart = clock; 
rng('default'); 
%% parameters 
maxiter = 100; 
...
%% main loop 
if verbose
    fprintf('\n ***** Jabobi Quantum Belief Propagation*********\n');
end
for iter = 1:maxiter 
    %% update message
    minq = 1;
    ...
    for k = 1:m
        ...
        i = I(k);
        j = J(k); 
        logSQM{j} = logSQM{j}+plogm(PTR(Qk,TI))-lam{k};
        logSQM{i} = logSQM{i}+plogm(PTR(Qk,TJ))-mu{k};
    end
    for k = 1:m
        Qk = Q{k};
        i = I(k);
        j = J(k);
        lamp{k} = -c{j}+logSQM{j}-(plogm(PTR(Qk,TI))-lam{k});
        mup{k} = -c{i}+logSQM{i}-(plogm(PTR(Qk,TJ))-mu{k});
    end
    for k = 1:m
        lam{k} = (lamp{k}+lamp{k}')/2;
        [P,S] = eig(lam{k});
        d = diag(S);
        sd = logsumexp(d,1);
        d = d-sd;
    ...
    for k = 1:n 
        if deg(k)>1
            ...
            if ~isempty(NI{k})
                j = NI{k};
                q{k} = PTR(Q{j},TJ); %+pert*eye(r);
                q{k} = q{k}/trace(q{k});
                q{k} = (q{k}+q{k}')/2;
                logq{k} = plogm(q{k});
            else
                i = NJ{k};
                q{k} = PTR(Q{i},TI); %+pert*eye(r);
                q{k} = q{k}/trace(q{k});
                q{k} = (q{k}+q{k}')/2;
                logq{k} = plogm(q{k});
            end
