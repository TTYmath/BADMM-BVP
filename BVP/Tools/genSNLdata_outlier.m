function [G,C,c,Sen,Anc,gnods] = genSNLdata_outlier(sig,R,N,na,t,ot)
rng('default');
Anc = rand(na,2);
Sen = rand(N,2);
%% sensor to sensor
...
for k = 1:m
    i = I(k); j = J(k);
    a = rand(1);
    logPhik = logPomat-abs(disn(i,j)*(a>=ot)+a*(a<ot)-dismat)/(2*sig)-log(sqrt(2*pi*sig^2));
...
for i = 1:N
    ...
    for j = 1:na
        if A(i,j) == 1
            a = rand(1);
            logphi{i} = logphi{i}+logPsaomat(:,j)-abs(dissan(i,j)*(a>=ot)+a*(a<ot)-dsamat(:,j))/(2*sig)-log(sqrt(2*pi*sig^2));
        end
    end
